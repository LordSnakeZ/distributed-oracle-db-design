...
-- Para PAIS
    ...
    FROM (
        ...
        UNION ALL
        SELECT pais_id
        FROM f_sss_pais_2@sssbdd_s2.fi.unam
    ) q1;
...
-- Para SUSCRIPTOR
    ...
    FROM (
        ...
        FROM (
            SELECT suscriptor_id
            FROM f_sss_suscriptor_3
            UNION ALL
            SELECT suscriptor_id
            FROM f_sss_suscriptor_4@sssbdd_s2.fi.unam
        ) q2
        UNION ALL
        SELECT suscriptor_id
        FROM f_sss_suscriptor_2
    ) q3;
...
-- Para Revista
    ...
    FROM (
        SELECT revista_id
        FROM f_sss_revista_1
        UNION ALL
        SELECT revista_id
        FROM f_sss_revista_2@sssbdd_s2.fi.unam
    ) q5;
...
-- Para Articulo Revista
    ...
    FROM (
        ...
        FROM f_sss_articulo_revista_1
        UNION ALL
        SELECT articulo_revista_id
        FROM f_sss_articulo_revista_2@sssbdd_s2.fi.unam
    ) q6;
...
-- Para Pago Suscriptor
    ...
    FROM (
        ...
        UNION ALL
        SELECT num_pago
        FROM f_sss_pago_suscriptor_2@sssbdd_s2.fi.unam
    ) q7;
